...
class MotionSim{
 /* system* sys; 
  ...
  MotionSim(ros::NodeHandle* _nh){
    ...
    traj_result = nh->subscribe("/motion/motion_command/result",5,&MotionSim::check_traj,this);
  ...
  void timercall(const ros::TimerEvent& event){
    while(ros::Time::now()-t0<tcurr+ros::Duration(DT)){};
    ...
    wpt_next.options.max_linear_speed = 50.0;
    wpt_next.options.max_linear_accel = 50.0;
    wpt_next.options.max_rotational_speed=10.0;
    wpt_next.options.max_rotational_accel = 10.0;
    wpt_next.options.corner_distance = 0.5;
    for (int i = 1; i<=Nwpt; i++){
      wpt_next.pose.pose = pose_init;
      wpt_next.pose.pose.position.x = pose_init.position.x - 0.05*sin(tcurr.toSec()/2+(i*DT/Nwpt));
      wpt_next.pose.pose.position.y = pose_init.position.y + 0.2*(cos(tcurr.toSec()/2+(i*DT/Nwpt))-1);
    ...
    traj.trajectory_options.tracking_options.goal_time_tolerance = DT/10;
    traj.trajectory_options.end_time = ros::Time::now()+ros::Duration(DT);
    traj.trajectory_options.path_interpolation_step = 0.;
    motiongoal.trajectory = traj;
    motionstop.command = "stop";
    ac->sendGoal(motionstop);
