...
data = read.csv("/home/kt-fitz/data/cpp/alldata2.csv")
...
apple<-subset(data,data$image==1&data$trial<31,select=subject:paa)
banana<-subset(data,data$image==2&data$trial<31,select=subject:paa)
house<-subset(data,data$image==3&data$trial<31,select=subject:paa)
umbrella<-subset(data,data$image==4,select=subject:paa)
